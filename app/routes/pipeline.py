...
async def process_contract(
    ...
    1. Uploads the document to local storage
    2. Extracts text (PDF/TXT/RTF)
    ...
    5. Returns pipeline_status: "ready_for_approval" or "issues_found"

    Form fields:
    - file: Contract document (PDF, DOC, DOCX, TXT, RTF, ODT)
    ...
    result = await run_pipeline(
        ...
        compare_contract_ids=compare_ids or None,
    )
...
async def send_for_approval(
    ...
    """
    Send a contract for approval after the pipeline passes.
    Requires the contract to have been analyzed first.
    """
    if not ObjectId.is_valid(contract_id):
        raise HTTPException(status_code=400, detail="Invalid contract ID")
    ...
    if not contract.get("ai_analysis"):
        raise HTTPException(
            status_code=400,
            detail="Contract must be analyzed before sending for approval. Run the pipeline first.",
        )
...
async def get_pipeline_status(
    ...
    return {
        ...
        "next_action": (
            "edit_and_resubmit" if has_issues else
            "send_for_approval"  if has_analysis else
            "upload_document"
        ),
    }
